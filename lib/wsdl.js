...
WSDL.prototype._initializeOptions = function (options) {
  ...
  if (options.overrideRootElement !== undefined) {
    ...
    // To handle XML/JSON date format differences.
  this.options.dateConversionCallback = options.dateConversionCallback;
};
...
WSDL.prototype.xmlToObject = function(xml, callback) {
  ...
  p.ontext = function(text) {
    ...
    if (name === 'int' || name === 'integer') {
      ...
      if(self.options.dateConversionCallback){
        value = self.options.dateConversionCallback(text);
      } else {
        value = new Date(text);
      }
    } else {
